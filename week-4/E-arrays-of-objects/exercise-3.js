...
var restaurantFinderApplication = {
  ...
  findAvailableRestaurants: function(numberOfPeople) {
    ...
    const availables = x =>
      x.numberOfCustomers + numberOfPeople <= x.totalSeats;
    return restaurants.filter(availables).map(getName);
  },
  findRestaurantServingDish: function(dishName) {
    const getName = x => x["name"];
    const availables = x => x.menu.includes(dishName);
    return restaurants.filter(availables).map(getName);
  },
  countNumberOfRestaurantsInArea: function(area) {
    const getName = x => x["name"];
    const availables = x => x.address.area.includes(area);
    const names = restaurants.filter(availables).map(getName);
    return names.length;
  }
};
